...
//eg: @import url("http://fonts.googleapis.com/css?family=Lato:400,700,700italic,900,400italic,300");
...
var buildTree = function(fileList, filePath){
	var parser = function(ret, uri){
		...
		if(typeof str === 'string'){
			...
			if(lastImported.match(/^http(s)?\:\/\//)){
				return '@import url("' + lastImported + '");';
			}
...
var reg = /\@charset([^;]*);|\/\*((.|\r|\n)*?)\*\//ig;
function getAllFiles(filePath,node,config,beCombined){
	var fileList = {};
	function getFiles(filePath,node,config,beCombined){
		node['root'] = filePath.replace(config.root,'');
		node['children'] = [];
		...
		if(fs.existsSync(filePath)){
			...
			return css.replace(impReg,function(){
				var imported = path.resolve(filePath,'..',arguments[3]);
				if(!beCombined[imported]){
					beCombined[imported] = true
					var subNode = {
						root : imported.replace(config.root,''),
						children : []
					};

					node['children'].push(subNode);
					return getFiles(imported,subNode,config,beCombined);
				}else{
					return '';
				}
			});
			...
			console.log(filePath + ' does not exist!111');
			return ' ';
		}
	}
	getFiles(filePath,node,config,beCombined);
	return fileList;
}

module.exports = function(filePath,config){
	var rootNode = {},beCombined = {};
	var fileList = getAllFiles(filePath,rootNode,config,beCombined);
	var tree = buildTree(fileList, filePath);
	
	if(config.debug){
		return '/*\n' + JSON.stringify(rootNode,null,3) + '\n*/ \n' + clear(tree).join('\n');
	}else{
		return tiny(clear(tree).join('\n')); 
	}
};tools